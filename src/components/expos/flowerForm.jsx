...
const FlowerForm = ({ onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('mongodb+srv://kinflower:kinflower@openclass.z9kv2xm.mongodb.net/?retryWrites=true&w=majority&appName=openclass', formData, {
        headers: {
...
const FlowerForm = ({ onSubmit }) => {
  const [flowerData, setFlowerData] = useState({
    ...
    couleur: '',
    categories: '',
    type: 'naturel', // Par défaut, le type est naturel
  });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit(flowerData); // Appeler la fonction onSubmit avec les données de la fleur
